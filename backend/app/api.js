...
const API_BASE_URL = 'http://localhost:5713/api'; 
...
export const analyzeAudioChunks = async (chunks, metadata) => {
  try {
    ...
    // 解析並回傳後端的 JSON（與 backend app/schemas.PredictResponse 一致）：
    // { labels, chunks: [{ index, probs?, top_k?, error? }], original_filename, sample_rate, warnings }
