 ...
 * @param {{ selectedEvent?: object | null, windowSec?: number, dict?: object }} [options]
 */
export function buildTimelineDecisionSupport(timeline, options = {}) {
  const { selectedEvent = null, windowSec = 5, dict = null } = options;
  ...
  if (selectedEvent) {
    ...
    const name = dict ? (selectedEvent.name?.[dict.htmlLang?.split('-')[0]] ?? nameEn) : nameEn;
    ...
    return {
      risk_analysis: fallbackDict
        ? (isBoundary
            ? formatMessage(fallbackDict.boundaryEventRisk, { name, peakTime: selectedEvent.peakTime, pct })
            : formatMessage(fallbackDict.generalEventRisk, { name, peakTime: selectedEvent.peakTime, pct, minPct }))
        : {
            zh: isBoundary
      ...
      action_recommendation: fallbackDict
        ? formatMessage(fallbackDict.eventFallbackAction, { timeRange })
        : {
  ...
  if (!speciesSummary.length || !events.length) {
    return {
      risk_analysis: fallbackDict
        ? formatMessage(fallbackDict.noTimelineEventsRisk, { minPct, boundaryPct })
        : {
      ...
      action_recommendation: fallbackDict
        ? fallbackDict.timelineFallbackAction
        : {
  ...
  const peakTime = top.peak_time != null ? `${top.peak_time}` : '—';

  return {
    risk_analysis: fallbackDict
      ? formatMessage(fallbackDict.timelineSummaryRisk, { eventCount: events.length, speciesCount, name, pct, peakTime })
      : {
          zh: `時間軸偵測到 ${events.length} 個物種事件、${speciesCount} 種鳥類；最高事件信心為 ${nameZh}（${pct}%，峰值 ${peakTime}s）。結果依反卷積時間軸與事件信心篩選，非逐窗投票聚合。`,
          en: `Timeline detected ${events.length} species event(s) across ${speciesCount} species; highest event confidence is ${nameEn} (${pct}%, peak ${peakTime}s). Based on deconvolved timeline filtering, not per-window vote aggregation.`,
        },
    action_recommendation: fallbackDict
      ? fallbackDict.timelineSummaryFallbackAction
      : {
